...
int main(int argc, char* argv[]) {
	...
	image = imread(argv[1]);	
		...
		//x y w h
		Rect roi(0,image.cols/1000,image.cols-1,image.rows - image.cols/8);// set the ROI for the image
  ...
  cvtColor( imgROI, src_gray, CV_BGR2GRAY );
   Mat src_thresh;
  threshold( imgROI, src_thresh, 30, 255,0 );
                  
   // Canny algorithm
	Mat contours;
	Canny(src_thresh,contours,50,250);
	Mat contoursInv;
...
sum[sumptr] = fabs( atan2(pt1.y - pt2.y,pt1.x - pt2.x) * 180.0 / PI);
sumptr++;
   ...
   // Display Canny image
	...
	//print
	int i;
	float max=-999,min=999,avg;
	for(i=0;i<=(sumptr-1);i++){
		if(sum[i] > max)
			max = sum[i];
...
//	min  +=90;
	avg = (min+max)/2.0;
//doen't work need to ponder...
//	std::cout << "\nCable angle:" << avg <<   std::endl;
}
